...
export const useMapStore = defineStore("map", () => {
  ...
  // ASN.1 CHOICE 타입 값 추출 (배열 ["key", value] 또는 객체 { key: value } 지원)
  const resolveChoice = (choice) => {
    if (Array.isArray(choice)) {
      return { key: choice[0], value: choice[1] };
    }
    if (choice && typeof choice === "object") {
      const key = Object.keys(choice)[0];
      return key ? { key, value: choice[key] } : null;
    }
    return null;
  };
  ...
  const extractDeltaValues = (deltaNode) => {
    // 배열 형태: ["node-XY4", { x: 613, y: -2301 }]
    if (Array.isArray(deltaNode)) {
      const val = deltaNode[1];
      if (val && val.x !== undefined && val.y !== undefined) {
        return { x: val.x, y: val.y };
      }
      return { x: 0, y: 0 };
    }
    // 객체 형태: { "node-XY4": { x: 613, y: -2301 } }
  ...
  const renderIntersectionData = (fileName) => {
    try {
      ...
      const mapData =
        state.intersectionData.value?.MapData || state.intersectionData.value;

      // 교차로 데이터 추출
      const intersection =
        state.intersectionData.value?.MapData?.intersections[0] ||
        state.intersectionData.value?.intersections[0];
      if (!intersection) {
  ...
  const addReferencePoint = (refPoint, id) => {
    ...
    const markerName = `${id.id}_${id.region}`;
  ...
  const processLanes = (laneSet, refPoint) => {
    ...
    laneSet.forEach((lane) => {
      try {
        let isIngress = lane.hasOwnProperty("ingressApproach");
        let isEgress = lane.hasOwnProperty("egressApproach");
        // approaches 정보가 없으면 directionalUse로 판단
        if (!isIngress && !isEgress) {
          const direction = parseDirectionalUse(
            lane.laneAttributes?.directionalUse,
          );
          isIngress = direction.isIngress;
          isEgress = direction.isEgress;
        }

        // laneType CHOICE 해석 (배열/객체 지원)
        const laneTypeChoice = resolveChoice(lane.laneAttributes?.laneType);
        const vehicleValue =
          laneTypeChoice?.key === "vehicle"
            ? laneTypeChoice.value?.value
            : lane.laneAttributes?.laneType?.vehicle?.value;
        const laneType = getLaneTypeDescription(vehicleValue) || "unknown";
        const laneColor = getLaneColor(isIngress, isEgress);

        let coordinates = [];

        // nodeList CHOICE 해석 (배열/객체 지원)
        const nodeListChoice = resolveChoice(lane.nodeList);

        if (nodeListChoice?.key === "nodes") {
          coordinates = processDirectNodes(nodeListChoice.value, refPoint);
        } else if (nodeListChoice?.key === "computed") {
          coordinates = processComputedNodes(
            nodeListChoice.value,
            laneSet,
            refPoint,
          );
        }
  ...
  // 참조 차선을 사용하는 노드 처리
  const processComputedNodes = (computedData, laneSet, refPoint) => {
    const refLaneId = computedData.referenceLaneId;
    const offsetX = computedData.offsetXaxis?.small || 0;
    const offsetY = computedData.offsetYaxis?.small || 0;

    const refLane = laneSet.find((l) => l.laneID === refLaneId);
    if (!refLane) return [];

    // 참조 차선의 nodeList에서 nodes 추출
    const refNodeList = resolveChoice(refLane.nodeList);
    if (refNodeList?.key !== "nodes") return [];

    const coordinates = [];
    let currentX = 0;
    let currentY = 0;

    refNodeList.value.forEach((node) => {
      const delta = extractDeltaValues(node.delta);
  ...
  // directionalUse 비트 파싱
  const parseDirectionalUse = (directionalUse) => {
    if (!directionalUse) return { isIngress: false, isEgress: false };

    const value = parseInt(directionalUse, 16);
    return {
      isIngress: (value & 0x80) !== 0, // 10000000
      isEgress: (value & 0x40) !== 0, // 01000000
    };
  };
